...
module.exports.saveUser = async (req, res) => {
  ...
  if (!name || !email || !password) {
    res.status(404).json({
      Message: "check name,email,password (all fields are mandotary)",
    });
  }
...
module.exports.resetPassword = async (req, res) => {
  ...
  const link = `https://comforting-souffle-617cbd.netlify.app/passwordreset/${randomString}`;

  user.resettoken = randomString;
  const updated = await UserModel.findByIdAndUpdate(user._id, user);
  if (updated) {
    res.status(201).json({
      Message: "Password reset link send to your mail id, Kindly check",
    });
  }
